...
module OpenapiClient
  class TranscriptionOutput
    ...
    def self.openapi_types
      {
        :'product' => :'TranscriptionOutputProduct',
        :'delivery' => :'Array<OneOfTranscriptionOutputDelivery>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::TranscriptionOutput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::TranscriptionOutput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @delivery.length > 1
        invalid_properties.push('invalid value for "delivery", number of items must be less than or equal to 1.')
      end

      if @delivery.length < 1
        invalid_properties.push('invalid value for "delivery", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @delivery.length > 1
      return false if @delivery.length < 1
      true
    end
    ...
    def delivery=(delivery)
      ...
      if delivery.length > 1
        fail ArgumentError, 'invalid value for "delivery", number of items must be less than or equal to 1.'
      end

      if delivery.length < 1
        fail ArgumentError, 'invalid value for "delivery", number of items must be greater than or equal to 1.'
      end
