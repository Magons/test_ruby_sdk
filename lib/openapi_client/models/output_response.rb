...
module OpenapiClient
  class OutputResponse
    # The package name. Standard - asr only. Pro - live asr only. (<b>&#9888;</b>Temporary solution)
    ...
    def self.openapi_types
      {
        :'product' => :'String',
        :'delivery' => :'Array<OneOfWebsocketResponseRtmpResponse>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::OutputResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::OutputResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      product_validator = EnumAttributeValidator.new('String', ["automatic", "professional"])
    ...
    def product=(product)
      validator = EnumAttributeValidator.new('String', ["automatic", "professional"])
